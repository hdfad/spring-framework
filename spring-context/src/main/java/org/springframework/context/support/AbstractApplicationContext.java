...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		...
		synchronized (this.startupShutdownMonitor) {
			 ...
			 *  2.添加对spel表达式解析器
			 *  3.添加属性编辑器
			 *  4.添加ApplicationContextAwareProcessor处理器
			 *  5.忽略依赖注入部分Aware接口
			 *  6.注册部分依赖
			 *  7.提供对LTW的支持
			 *  8.注册系统环境
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		/**
		 * 设置类加载器
		 * 类加载起优先获取当前线程类加载器，没有再获取ClassUtils ClassLoader，没有再获取bootstrap ClassLoader
		 */
		beanFactory.setBeanClassLoader(getClassLoader());

		/**
		 * 是否忽略对spel表达式的支持，默认不忽略
		 * 不忽略则将el表达式解析器添加到beanFactory中
		 */
		if (!shouldIgnoreSpel) {
		...
		/**
		 * 将监听器添加到后置处理器中，完成beanFactory对监听器的支持
		 */
		...
		//LOAD_TIME_WEAVER_BEAN_NAME：运行期间织入  todo LTW
	 ...
	 * 			ApplicationContext implement ApplicationEventPublisher
	 * ApplicationListener：事件监听
	 * initApplicationEventMulticaster：初始化事件监听器SimpleApplicationEventMulticaster，事件的执行在Bean初始化完成之后通过上下文调用publishEvent方法执行
	 */
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// LoadTimeWeaverAware 对象初始化,LoadTimeWeaverAware主要用于加载Spring Bean时织入第三方模块，如AspectJ
	...
	protected void finishRefresh() {
		...
		/**
		 * 初始化DefaultLifecycleProcessor，并注册到bean工厂中
		 */
